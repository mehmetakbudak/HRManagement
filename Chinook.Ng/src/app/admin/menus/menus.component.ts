...
import { AlertService, alertType, defaultMessageType } from 'src/app/services/alert.service';
...
export class MenusComponent implements OnInit {
  menuSettings = [{ text: 'Düzenle', value: 'edit' }, { text: 'Sil', value: 'delete' }];
  menuItemSettings = [{ text: 'Alt Menü Elemanı Ekle', value: 'addSubMenuItem' }, { text: 'Yukarı Taşı', value: 'up' }, { text: 'Aşağı Taşı', value: 'down' }, { text: 'Düzenle', value: 'edit' }, { text: 'Sil', value: 'delete' }];
  loading = false
  ...
  bindGrid() {
    this.appService.getApi(this.appService.menu).then((res: any) => {
      this.data = res;
    })
  }
  ...
  saveMenu() {
    ...
    if (this.formMenu.valid) {
      if (this.formMenu.value.id) {
        this.appService.put(this.appService.menu, this.formMenu.value).then(res => {
          this.bindGrid()
        ...
        this.appService.post(this.appService.menu, this.formMenu.value).then(res => {
          this.bindGrid()
          this.isMenuVisible = false;
